...
export default {
  ...
  computed: {
    specOptions() {
  // Ensure all IDs from the spec array are included in the dropdown
  return this.spec.map((spec) => spec.id).sort((a, b) => a - b);
},

  selectedSpec() {
    // Match the selected spec_id with the spec array based on id
  ...
  this.$watch('msg', (newMsg) => {
    if (newMsg && newMsg.payload) {
      const { spec, latest_spec_id } = newMsg.payload;
      if (spec) {
        this.spec = spec;
        // Default to the latest `id` if none is selected
        const latestSpec = this.spec.reduce((max, item) => (item.id > max.id ? item : max), { id: -1 });
        if (!this.formData.spec_id && latestSpec.id >= 0) {
          this.formData.spec_id = latestSpec.id;
        }
      }
      if (latest_spec_id !== undefined) {
        this.latest_spec_id = latest_spec_id;
      }
    }
  });
},
